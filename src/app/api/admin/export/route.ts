...
export async function GET(request: Request) {
    ...
    try {
        ...
        const csvRows = signups.map((s: { id: string; email: string; reason: string | null; category: string | null; ip: string | null; userAgent: string | null; createdAt: Date }) => {
        ...
        return new NextResponse(csvContent, {
            headers: {
                ...
                'Content-Disposition': 'attachment; filename="framerights-waitlist.csv"',
            },
        });
    } catch (error) {
        console.error(error);
